...
def main():
    ...
    # build & run infer.py
    infer_cmd = [sys.executable, infer_script, '-bb', args.backbone, '-dpt', str(args.dropout_prob_trans), '-dpb', str(args.dropout_prob_backbone), '-dpr', str(args.dropout_prob_rot), '-b', str(args.batch_size), '-dp', str(args.dropout_prob), '-sn', str(args.sample_nbr), '-ccw', str(args.complexity_cost_weight), '-bt', str(args.bayesian_type), '-is', str(args.input_sigma)]
